...
export default class Crawler {
  ...
  snap() {
    ...
    return snapshot(this.protocol, this.host, urlPath, this.snapshotDelay).then(window => {
      ...
      if (Boolean(window.dulcet_snapshot_state)) {
        const stateJSON = JSON.stringify(window.dulcet_snapshot_state)
        const script = window.document.createElement('script')
        script.innerHTML = `window.dulcet_snapshot_state = JSON.parse('${stateJSON}');`
